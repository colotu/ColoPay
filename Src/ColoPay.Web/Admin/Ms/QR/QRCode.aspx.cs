...
namespace ColoPay.Web.Admin.Ms.QR
{
    public partial class QRCode : PageBaseAdmin
    {
        ...
        protected void btnGen_Click(object sender, EventArgs e)
        {
            ...
            //{
            //    YSWL.Common.MessageBox.ShowSuccessTip(this, "请输入二维码内容!");
            //    return;
            //}
            ...
            try
            {
                ...
                {
                    //重新生成二维码
                    websiteUrl = "http://" + YSWL.Common.Globals.DomainFullName +
                                 string.Format(baseURL, YSWL.Common.Globals.UrlEncode(websiteUrl));
                ...
                {
                    //重新生成二维码
                    androidUrl = "http://" + YSWL.Common.Globals.DomainFullName +
                                 string.Format(baseURL, YSWL.Common.Globals.UrlEncode(androidUrl));
                    webClient.DownloadFile(androidUrl, androidImg);
                }
            }
            catch (Exception ex)
            {
                YSWL.Common.MessageBox.ShowFailTip(this, "二维码生成失败! " + ex.Message);
                return;
            }
            //string content = YSWL.Common.Globals.UrlEncode(txtContent.Text);
            ...
            ShowInfo();
            YSWL.Common.MessageBox.ShowSuccessTip(this, "二维码生成成功!");
        }
    }
}
