...
namespace ColoPay.SQLServerDAL.Members
{
    ...
    public partial class UsersExp 
    {
        ...
        /// 得到一个对象实体
        /// </summary>
        public ColoPay.Model.Members.UsersExpModel GetModel(int UserID)
        {
        ...
        /// 得到一个对象实体
        /// </summary>
        public ColoPay.Model.Members.UsersExpModel DataRowToModel(DataRow row)
        {
            ColoPay.Model.Members.UsersExpModel model = new ColoPay.Model.Members.UsersExpModel();
            if (row != null)
            {
        ...
        public int GetUserCountByKeyWord(string NickName)
        {
            ...
            {
                strSql.Append("AND NickName LIKE '%" + YSWL.Common.InjectionFilter.SqlFilter(NickName) + "%'");
            }
            object obj = DBHelper.DefaultDBHelper.GetSingle(strSql.ToString());
            if (obj == null)
            {
