...
using YSWL.Common;
namespace ColoPay.SQLServerDAL.Members
{
    /// <summary>
    /// 数据访问类:Users
    /// </summary>
    public partial class Users
    {
        ...
        public ColoPay.Model.Members.Users GetModel(int UserID)
        {
            ...
            ColoPay.Model.Members.Users model = new ColoPay.Model.Members.Users();
        ...
        /// 得到一个对象实体
        /// </summary>
        public ColoPay.Model.Members.Users DataRowToModel(DataRow row)
        {
            ColoPay.Model.Members.Users model = new ColoPay.Model.Members.Users();
            if (row != null)
            {
        ...
        public DataSet GetList(string type, string keyWord)
        {
            ...
            {
                //过滤SQL注入
                strSql.AppendFormat(" AND UserName LIKE '%{0}%' ", YSWL.Common.InjectionFilter.SqlFilter(keyWord));
            }
            return DBHelper.DefaultDBHelper.Query(strSql.ToString());
        }
        ...
        public DataSet GetListEXByType(string type, string keyWord = "")
        {
            ...
            if (!string.IsNullOrEmpty(type))
            {
                ...
                strWhere.Append("  UserType='" + YSWL.Common.InjectionFilter.SqlFilter(type) + "'");
            }
            if (!string.IsNullOrEmpty(keyWord))
            {
                ...
                strWhere.AppendFormat("  UserName LIKE '%{0}%' ", YSWL.Common.InjectionFilter.SqlFilter(keyWord));
            }
        ...
        public DataSet GetSearchList(string type, string StrWhere = "")
        {
            ...
            if (!string.IsNullOrEmpty(type))
            {
                ...
                strWhere2.Append("  UserType='" + YSWL.Common.InjectionFilter.SqlFilter(type) + "'");
            }
        ...
        public DataSet GetCustList(int UserId, int IsAct = -1, string KeyWord = "",string startDate="")
        {
            ...
            if (!String.IsNullOrWhiteSpace(KeyWord))
            {
                strSql.AppendFormat(" and ( UserName like '%{0}%'   or  NickName like '%{0}%'  ) ", YSWL.Common.InjectionFilter.SqlFilter(KeyWord));
            }
            if (IsAct == 0)
            {
