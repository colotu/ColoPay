...
namespace YSWL.DBUtility
{
    ...
    public class SAASInfo
    {
        ...
        public static bool AppIsOpenCache(string tag, int enterpeiseId)
        {
            ...
            {
                Log.LogHelper.AddTextLog("获取是否开启"+ tag + "失败" + ex.Message, "错误信息：" + ex.StackTrace);
                throw;
            }
        ...
        public static readonly string[] DOMAIN_SAAS_SEC = new string[] { "saas.ys56.com", "saas.yuns56.cn", "saasadmin.ys56.com", "localhost" };
        ...
        public static int GetSAASEntIdByMallDomain(string domain)
        {
            ...
            try
            {
                ...
                if (enterpriseId == -1)
                {
                    //DB加载
                    StringBuilder strSql = new StringBuilder();
                    strSql.Append(" select top 1 EnterpriseID FROM Ms_Enterprise where CustomMallDomain=@CustomMallDomain");
                    SqlParameter[] parameters =
                    {
                        new SqlParameter("@CustomMallDomain", SqlDbType.NVarChar)
                    };
                    parameters[0].Value = domain;
                    object obj = YSWL.DBUtility.ConnectionStrManage.GetSingle(strSql.ToString(), parameters);
                    enterpriseId = Globals.SafeInt(obj, -1);
