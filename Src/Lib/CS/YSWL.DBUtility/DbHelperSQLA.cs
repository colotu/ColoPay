...
namespace YSWL.DBUtility
{
    ...
    /// </summary>
    public class DbHelperSQLA : DBBase
    {
        ////数据库连接字符串(web.config来配置)
        //public override string connectionString = PubConstant.ConnectionString;
        //public DbHelperSQL()
        //{
        //}
        ...
        public override int ExecuteSql(string SQLString, params SqlParameter[] cmdParms)
        {
                ...
                SqlConnection connection = new SqlConnection(GetConnectionStr()))
            {
                using (SqlCommand cmd = new SqlCommand())
                {
                    ...
                    {
                        Log.LogHelper.AddErrorLog("数据库执行错误："+e.Message,e.StackTrace);
                        throw e;
                    }
                }
            }
        ...
        public override int ExecuteSqlTran4Indentity(System.Collections.Generic.List<CommandInfo> cmdList)
        {
            using (SqlConnection conn = new SqlConnection(GetConnectionStr()))
            {
                conn.Open();
        ...
        public override int ExecuteSqlTran4Indentity(System.Collections.Generic.List<CommandInfo> cmdList, SqlTransaction trans)
        {
            ...
            foreach (CommandInfo commandInfo in cmdList)
            {
                ...
                try
                {
                    ...
                    {
                        //未执行成功抛出异常, 由外部进行事务回滚
                        throw new SqlExecutionException("DBHelper.DefaultDBHelper.ExecuteSqlTran4Indentity - [" + cmd.CommandText + "] 未执行成功!");
                    }
                    foreach (SqlParameter parameter in cmdParms)
                    {
                        if (parameter.Direction == ParameterDirection.Output)
        ...
        private void PrepareCommand(SqlCommand cmd, SqlConnection conn, SqlTransaction trans, string cmdText, SqlParameter[] cmdParms)
        {
            ...
            catch (Exception ex)
            {
                foreach (SqlParameter parameter in cmdParms)
                {
                    ...
                    valueStr += parameter.Value == DBNull.Value ? "" : parameter.ParameterName + ":" + parameter.Value.ToString()+"";
                }
                YSWL.Log.LogHelper.AddErrorLog(ex.Message+"__" + Common.CallContextHelper.GetAutoTag(), "cmdText："+ cmdText+"—参数："+ valueStr);
                throw ex;
            }
        ...
        /// <param name="tableName"></param>
        /// <returns></returns>
        public override void RunProcedure(string storedProcName, IDataParameter[] parameters, DataSet dataSet, string tableName)
        {
            using (SqlConnection connection = new SqlConnection(GetConnectionStr()))
            {
                connection.Open();
        ...
        private SqlParameter CreateParam(string ParamName, SqlDbType DbType,
        ...
        public override SqlParameter CreateInParam(string ParamName,
        ...
        /// <param name="Size">参数大小</param>
        /// <returns>新的 parameter 对象</returns>
        public override SqlParameter CreateOutParam(string ParamName,
        ...
        /// <param name="Size">参数大小</param>
        /// <returns>新的 parameter 对象</returns>
        public override SqlParameter CreateInputOutParam(string ParamName,
            SqlDbType DbType, int Size, object Value)
        {
        ...
        /// <param name="Size">参数大小</param>
        /// <returns>新的 parameter 对象</returns>
        public override SqlParameter CreateReturnParam(string ParamName,
            SqlDbType DbType, int Size)
        {
