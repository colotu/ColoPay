/**  版本信息模板在安装目录下，可自行修改。
* Agent.cs
*
* 功 能： N/A
* 类 名： Agent
*
* Ver    变更日期             负责人  变更内容
* ───────────────────────────────────
* V0.01  2019/1/3 21:18:53   N/A    初版
*
* Copyright (c) 2012 Maticsoft Corporation. All rights reserved.
*┌──────────────────────────────────┐
*│　此技术信息为本公司机密信息，未经本公司书面同意禁止向第三方披露．　│
*│　版权所有：动软卓越（北京）科技有限公司　　　　　　　　　　　　　　│
*└──────────────────────────────────┘
*/
using System;
using System.Data;
using System.Collections.Generic;
using YSWL.Common;
using ColoPay.Model.Pay;
namespace ColoPay.BLL.Pay
{
	/// <summary>
	/// Agent
	/// </summary>
	public partial class Agent
	{
		private readonly ColoPay.DAL.Pay.Agent dal=new ColoPay.DAL.Pay.Agent();
		public Agent()
		{}
		#region  BasicMethod

		/// <summary>
		/// 得到最大ID
		/// </summary>
		public int GetMaxId()
		{
			return dal.GetMaxId();
		}

		/// <summary>
		/// 是否存在该记录
		/// </summary>
		public bool Exists(int AgentId)
		{
			return dal.Exists(AgentId);
		}

		/// <summary>
		/// 增加一条数据
		/// </summary>
		public int  Add(ColoPay.Model.Pay.Agent model)
		{
			return dal.Add(model);
		}

		/// <summary>
		/// 更新一条数据
		/// </summary>
		public bool Update(ColoPay.Model.Pay.Agent model)
		{
			return dal.Update(model);
		}

		/// <summary>
		/// 删除一条数据
		/// </summary>
		public bool Delete(int AgentId)
		{
			
			return dal.Delete(AgentId);
		}
		/// <summary>
		/// 删除一条数据
		/// </summary>
		public bool DeleteList(string AgentIdlist )
		{
			return dal.DeleteList(YSWL.Common.Globals.SafeLongFilter(AgentIdlist,0) );
		}

		/// <summary>
		/// 得到一个对象实体
		/// </summary>
		public ColoPay.Model.Pay.Agent GetModel(int AgentId)
		{
			
			return dal.GetModel(AgentId);
		}

		/// <summary>
		/// 得到一个对象实体，从缓存中
		/// </summary>
		public ColoPay.Model.Pay.Agent GetModelByCache(int AgentId)
		{
			
			string CacheKey = "AgentModel-" + AgentId;
			object objModel = YSWL.Common.DataCache.GetCache(CacheKey);
			if (objModel == null)
			{
				try
				{
					objModel = dal.GetModel(AgentId);
					if (objModel != null)
					{
						int ModelCache = YSWL.Common.ConfigHelper.GetConfigInt("ModelCache");
						YSWL.Common.DataCache.SetCache(CacheKey, objModel, DateTime.Now.AddMinutes(ModelCache), TimeSpan.Zero);
					}
				}
				catch{}
			}
			return (ColoPay.Model.Pay.Agent)objModel;
		}

		/// <summary>
		/// 获得数据列表
		/// </summary>
		public DataSet GetList(string strWhere)
		{
			return dal.GetList(strWhere);
		}
		/// <summary>
		/// 获得前几行数据
		/// </summary>
		public DataSet GetList(int Top,string strWhere,string filedOrder)
		{
			return dal.GetList(Top,strWhere,filedOrder);
		}
		/// <summary>
		/// 获得数据列表
		/// </summary>
		public List<ColoPay.Model.Pay.Agent> GetModelList(string strWhere)
		{
			DataSet ds = dal.GetList(strWhere);
			return DataTableToList(ds.Tables[0]);
		}
		/// <summary>
		/// 获得数据列表
		/// </summary>
		public List<ColoPay.Model.Pay.Agent> DataTableToList(DataTable dt)
		{
			List<ColoPay.Model.Pay.Agent> modelList = new List<ColoPay.Model.Pay.Agent>();
			int rowsCount = dt.Rows.Count;
			if (rowsCount > 0)
			{
				ColoPay.Model.Pay.Agent model;
				for (int n = 0; n < rowsCount; n++)
				{
					model = dal.DataRowToModel(dt.Rows[n]);
					if (model != null)
					{
						modelList.Add(model);
					}
				}
			}
			return modelList;
		}

		/// <summary>
		/// 获得数据列表
		/// </summary>
		public DataSet GetAllList()
		{
			return GetList("");
		}

		/// <summary>
		/// 分页获取数据列表
		/// </summary>
		public int GetRecordCount(string strWhere)
		{
			return dal.GetRecordCount(strWhere);
		}
		/// <summary>
		/// 分页获取数据列表
		/// </summary>
		public DataSet GetListByPage(string strWhere, string orderby, int startIndex, int endIndex)
		{
			return dal.GetListByPage( strWhere,  orderby,  startIndex,  endIndex);
		}
        /// <summary>
        /// 分页获取数据列表
        /// </summary>
        //public DataSet GetList(int PageSize,int PageIndex,string strWhere)
        //{
        //return dal.GetList(PageSize,PageIndex,strWhere);
        //}

        #endregion  BasicMethod
        #region  ExtensionMethod

        public int GetAgentId(string userName)
        {
            return dal.GetAgentId(userName);
        }

        #endregion  ExtensionMethod

        #region  周 20190104增加  用户名是否重复
        /// <summary>
        /// 是否用户名是否存在
        /// </summary>
        public bool ExistsUsername(string strUsername)
        {
            return dal.ExistsUsername(strUsername);
        }

        /// <summary>
        /// 修改用户名是否存在
        /// </summary>
        public bool ExistsUsername(string Agentid, string strUsername)
        {
            return dal.ExistsUsername(Agentid, strUsername);
        }

        /// <summary>
        /// 输入用户名，获得姓名
        /// </summary>
        /// <param name="userName"></param>
        /// <returns></returns>
        public string GetNameByusername(string userName)
        {
            return dal.GetNameByusername(userName);
        }

        #endregion

        #region  获取代理商的ID
        public int GetAgentID(string userName)
        {
            return dal.GetAgentID(userName);
        }
        # endregion
    }
}

